...
use dapr_wasm_components_e2e::{compose, grpc_provider_path, microservice_path};
...
fn microservices_through_real_dapr_grpc_on_spin() {
    ...
    // One composed artifact (microservice + wasi-grpc provider); both
    // instances run it, differing only by manifest env.
