...
use dapr_wasm_components_e2e::{compose, http_inbound_path, http_outbound_path, microservice_path};
...
fn microservices_through_real_dapr() {
    ...
    // One composed artifact (microservice + the wasi-http outbound *and*
    // inbound providers — `plug_full`); both instances run it, differing only
    // by env. The app is a reactor, so it needs both directions.
