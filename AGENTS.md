...
- WIT stays **sync** (no `async` functions, no `stream`/`future`). Worlds: **`app`** (imports building blocks, exports `*-callback`), **`dapr-outbound`** (exports building blocks) and **`dapr-inbound`** (imports `*-callback`); inbound is now typed through the inbound provider, not the app's own `wasi:http/incoming-handler`. Interface changes must be mirrored in: the relevant provider components (`wasi-http-outbound`/`wasi-http-inbound`/`wasi-grpc-outbound`), the **`dapr-app` SDK** (`app-sdk/`), the e2e mock + tests, the demo apps, and the README interface tables. Bump the package version in `components/wit/types.wit` (CI checks it against release tags).
